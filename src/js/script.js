...
(function() {
  ...
  class WeatherMap {
    ...
    fade() {
      ...
      if (shouldFade) {
        this.el.parentElement.classList.add("faded");
      } else {
        this.el.parentElement.classList.remove("faded");
      }
    }

    resize() {
      // Set Container Width
      const globeWidth = Math.min(window.innerWidth * 0.7, window.innerHeight * 0.7);
      this.el.parentElement.style.width = `${globeWidth}px`;
      ...
      } else if (globeWidth > 400 && globeWidth < 600) {
        this.data.resolution = 3;
      } else {
